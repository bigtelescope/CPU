...
int Stack::StackHash()
{
	...
	sum ^= size;
/*
	И - ненадежно. Теряется уж очень много инфы. Почти всегда у тебя 
	amount не вылазит битов за 10-15, остальные будут нули. Лучше уж 
	ксорить, мне кажется...
 */
	sum &= amount;
/*
	Ну заюзайте же сам указатель)
	Иначе выстрелом в объект метким можно сделать так, что я подменю
	указатель на невалидный, и хеш не просто не заметит, что не все 
	ок - он упадет на сегфолт.
 */
	return sum;
}
...
{
/*
	Надо бы проверить сначала data. Формально ведь можно вызвать эту
	функцию в любом месте. Дампы нужны не только в случае падения
	бывают. Перед любой опасной операцией хорошо бы проверить, не 
	будет ли ошибки там сдуру.
 */
